# PLUR  ☮️ 🤩
# credit:https://github.com/VitamintK/pluribus-hand-parser
...
def read_directory(directory_path):
    ...
    for f in onlyfiles:
        ...
        sessions.append(read_hands_from_file(
            join(directory_path, f), session_number))
    return sessions
...
class Hands(list):
    ...
    def get_poker_stars_df(self):
        return pd.DataFrame([h.get_poker_stars_dict() for h in self])
...
class Hand:
    ...
    def get_action_groups(action_string):
        ...
        action_strings = re.match("^{}$".format(
            '([crf]\\d*)'*number_of_actions), action_string)
    ...
    def _parse_register_action(self, player_seat, action_type, amount=None):
        ...
        if action_type in ['sb', 'bb', 'b']:
            ...
            self._parse_max_money_in_pot = max(
        ...
        self.actions[self._parse_cur_street].append(Action(
            self.player_ids[player_seat], action_type, total_amount, marginal_amount, is_all_in))
        if action_type == 'ca':
    ...
    def parse(self):
        ...
        if len(split) != 6:
            raise ValueError(
                "hand_text for this Hand does not contain 6 parts: {}".format(hand_text))
        if split[0] != 'STATE':
            raise ValueError(
                "hand_text for this Hand is not a single line of text starting with 'STATE:' {}".format(hand_text))
        ...
        self.player_hole_cards = [Cards(hole_cards)
                                  for hole_cards in cards_by_street[0].split('|')]
        self.community_cards_by_street = [
            Cards(community_cards) for community_cards in cards_by_street[1:]]
        ...
        self.actions = [[] for i in range(
            len([actions for actions in actions_by_street if actions])+1)]
        # blind:
        ...
        self.pot = sum(x for x in self._parse_money_in_pot) - \
            (self.uncalled_amount if self.uncalled_amount else 0)
        self.parsed = True
    ...
    def get_poker_stars_str(self, pre=''):
        ...
        session = int(''.join(x for x in self.session if x.isdigit())
                      ) if self.session else 0
        # july 17, 2019 - the day that poker died
        base_time = datetime.datetime(2019, 7, 11)
        time = base_time + \
            datetime.timedelta(seconds=1000*session + int(self.hand_number))
        hh = ''
        hh += "PokerStars Hand #{}: Hold'em No Limit (50/100) - {}\n".format(
            1000*session + int(self.hand_number), time.strftime("%Y/%m/%d %H:%M:%S ET"))
        ...
        for player_id, hole_cards in zip(self.player_ids, self.player_hole_cards):
            hh += 'Dealt to {} {}\n'.format(player_id,
                                            hole_cards.get_poker_stars_str())
        ...
        for street_no, community_cards in enumerate(self.community_cards_by_street):
            ...
            hh += '*** {} *** {}\n'.format(titles[street_no],
                                           ' '.join(community_cards_strs))
        ...
        if len(self.community_cards_by_street) == 3 and self.actions[-1][-1].action_type in ['ca', 'ch']:
            ...
            for player_no in self.winners:
                hh += "{}: shows {}\n".format(
                    self.player_ids[player_no], self.player_hole_cards[player_no].get_poker_stars_str())
        else:
            saw_showdown = False
        for player_no in self.winners:
            ...
            if self.uncalled_amount:
                hh += "Uncalled bet ({}) returned to {}\n".format(
                    self.uncalled_amount, player_id)
            hh += "{} collected {} from pot\n".format(
                player_id, self.pot/len(self.winners))
        if len(self.winners) == 0:
            ...
            hh += "{} collected {} from pot\n".format(
                self.player_ids[0], self.pot/2)
            hh += "{} collected {} from pot\n".format(
                self.player_ids[1], self.pot/2)
        hh += "*** SUMMARY ***\n"
        hh += "Total pot {} | Rake 0\n".format(self.pot)
        if len(self.community_cards_by_street) > 0:
            hh += "Board {}\n".format(sum(self.community_cards_by_street,
                                      Cards('')).get_poker_stars_str())
        for seat, player_id in enumerate(self.player_ids):
            ...
            if saw_showdown and self.players_active[seat]:
                winloss_string = "won ({})".format(
                    self.pot/len(self.winners)) if self.profits[seat] > 0 else "lost"
                hh += "Seat {}: {} showed {} and {}\n".format(
                    seat+1, player_id, self.player_hole_cards[seat].get_poker_stars_str(), winloss_string)
            # TODO: add Seat {}: {} strings for folding
        return hh

    def get_poker_stars_dict(self, pre=''):

        data = {
            'community_cards_flop': self.community_cards_by_street[0].get_poker_stars_str().strip('[').strip(']').split() if len(self.community_cards_by_street) > 0 else None,
            'community_cards_turn': self.community_cards_by_street[1].get_poker_stars_str().strip('[').strip(']').split() if len(self.community_cards_by_street) > 1 else None,
            'community_cards_river': self.community_cards_by_street[2].get_poker_stars_str().strip('[').strip(']').split() if len(self.community_cards_by_street) > 2 else None,
            'winners': [x + 1 for x in self.winners],
            'uncalled_amount': self.uncalled_amount,
            'pot': self.pot,
        ...
        for i in range(len(self.player_ids)):
            data['player{}_card'.format(
                i+1)] = self.player_hole_cards[i].get_poker_stars_str().strip('[').strip(']').split()
            data['player{}_action'.format(i+1)] = [
                action.get_poker_stars_str().split(':')[1].strip() for action_list in self.actions[1:] for action in action_list if self.player_ids[i] in action.get_poker_stars_str()]
...
class Action:
    ...
    def get_poker_stars_str(self):
        action_type_to_string = {'b': 'bets', 'f': 'folds', 'ca': 'calls', 'ch': 'checks',
                                 'r': 'raises', 'sb': 'posts small blind', 'bb': 'posts big blind'}
        if self.action_type == 'r':
            ans = "{}: {} {} to {}".format(
                self.player_id, action_type_to_string[self.action_type], self.marginal_amount, self.amount)
        elif self.action_type in ['sb', 'bb', 'b']:
            ans = "{}: {} {}".format(
                self.player_id, action_type_to_string[self.action_type], self.amount)
        elif self.action_type in ['ca']:
            ans = "{}: {} {}".format(
                self.player_id, action_type_to_string[self.action_type], self.marginal_amount)
        else:
            ans = "{}: {}".format(
                self.player_id, action_type_to_string[self.action_type])
        if self.is_all_in:
            ans += " and is all-in"
        return ans
...
if __name__ == '__main__':
    ...
    # directory = './' + \
    #     (input("Enter the directory path (relative to current path): "))
    # sessions = read_directory(directory)
    ...
    log_files = [f for f in listdir('./5H1AI_logs') if f.endswith('.log')]

    dfs = []

    for log_file in log_files:
        hands = read_hands_from_file(f'./5H1AI_logs/{log_file}', 0)
        df = hands.get_poker_stars_df()
        dfs.append(df)

    df = pd.concat(dfs, ignore_index=True)
    df.to_csv('out/5H1AI_logs.csv', index=False)
